...
let createNavigation = () => {
    ...
    let homeData = links[0];
    let homeLi = createTextElement({ element: 'li', text: homeData.name, idName: 'homeLink' });
    ...
    let linksCont = createPageLinksCont(links.slice(1)); // Returns array without first element
    // Append container to ul
    navUl.appendChild(linksCont);
    // Create "hamburger" element for when screen is too small
    let bars = createElement({
        element: "i",

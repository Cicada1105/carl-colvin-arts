...
const createHeaderContent = (data) => {
    ...
    let leftReed = createImageElement({ src: `${IMG_PATH}reed.png`, alt: 'Reed silhouette', className: 'reed_silhouette', idName: 'leftReed' });
    let introHeader = createTextElement({ element: 'h3', text: data.header });
    let rightReed = createImageElement({ src: `${IMG_PATH}reed.png`, alt: 'Reed silhouette', className: 'reed_silhouette', idName: 'rightReed' });

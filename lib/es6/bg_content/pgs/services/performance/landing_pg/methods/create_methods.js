...
const createSVGFilter = (data) => {
    ...
    data.filter.stdDeviation && filter.appendChild(createFeGaussianBlur(data.filter));
    // Add additional core attributes if provided
    UTILITY.setCoreAttributes(filter, data);
    return filter;
};
const createFeGaussianBlur = (data) => {
    let gaussBlurEl = document.createElementNS("http://www.w3.org/2000/svg", "feGaussianBlur");

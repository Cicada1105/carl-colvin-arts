...
const loadContactPage = () => {
  ...
  _data.formData.form.forEach(input => {
    let formInput = (0, _load_methods.loadInputRow)(input); // Append input to form container
...
const submitForm = event => {
  ...
  if (fieldsCompleted()) {
    ...
    let body = document.querySelector("#message"); // Replace button with image
...
const fieldsCompleted = () => {
  ...
  for (let input of inputs) {
    if (input.firstChild.value === "") {
      ...
      formCompleted = false;
    } else input.lastChild.innerHTML = "*";
  }
